...
import { Chart, ChartArea, ChartTooltipItem, ChartTooltipModelBody, InteractionMode  } from 'chart.js';
...
export class GraphLineChartComponent implements OnInit {
  ...
  // chartAreaBorder = {
  //   id: 'chartAreaBorder',
  //   beforeDraw: (chart: Chart, easing: number) => {
  //     const { ctx, chartArea }: { ctx: CanvasRenderingContext2D; chartArea: ChartArea } = chart;
  //     ctx.save();
  ...
  //     
  //     if (tooltip && tooltip._active && tooltip._active.length && chart.config.type === 'line') {
  //       const activePoint = tooltip._active[0];
  //       const x = activePoint._model.x; // Get the x position of the active tooltip
  //       const yScale = chart.scales['y-axis-0']; // Get the y-axis scale
  // 
  //       if (yScale) {
  ...
  //     
  //     // Override the fit method to add space below the legend
    ...
    return {
      ...
      layout: {
        padding: {
          top: 0,
          right: 0,
          bottom: 20, // Add space under the legend
          left: 0
        }
      },      
        ...
        yAxes: [{
          ...
          ticks: {
            ...
            callback: (value: number) => {
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); // This will add commas
            },
            padding: 20 
          }
        }]
      ...
      legend: {
        ...
        labels: {
          fontColor: '#0F131B',
          usePointStyle: true,  // Use point style for circular markers
          boxWidth: 5,         // Set the width of the legend marker
          fontSize: 14,         // Set the font size of the legend labels
          padding: 20           // Optional: set padding between legend items
        }
      },      
    };
  }
  ...
  private getTooltipOptions(graphData: any[]) {    
    return {      
      ...
      external: (context: any) => {
        ...
        if (tooltipModel.body) {
          ...
          const bodyLines:ChartTooltipModelBody[]  = tooltipModel.body.map((item: ChartTooltipModelBody) => item.lines);
          let innerHtml = '<thead>';
          ...
          console.log('bodyLines', bodyLines, bodyLines.length);
          bodyLines.forEach((body: ChartTooltipModelBody, i: number) => {
